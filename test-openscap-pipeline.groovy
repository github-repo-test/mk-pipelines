 ...
 *  XCCDF_BENCHMARKS            List of XCCDF benchmarks filenames (e.g. manila/openstack_manila-xccdf.xml,horizon/openstack_horizon-xccdf.xml)
 *  XCCDF_PROFILE               The XCCDF profile to evaluate (default 'default')
...
node('python') {
    ...
    def xccdfProfile = XCCDF_PROFILE ?: 'default'
    def benchmarksArray = XCCDF_BENCHMARKS.tokenize(',')
    ...
    stage ('Run openscap xccdf evaluation and attempt to upload the results to a dashboard') {
        ...
        for (minion in liveMinions) {
            ...
            // Set necessary variables for uploading the scanning results to the dashboard
            if (UPLOAD_TO_DASHBOARD.toBoolean()) {
                if (common.validInputParam('DASHBOARD_API_URL')) {
                    ...
                    environments = common.parseJSON(http.sendHttpGetRequest("${DASHBOARD_API_URL}/environment/"))
                    ...
                    if (! cloudId ) {
                        ...
                        cloudId = common.parseJSON(http.sendHttpPostRequest("${DASHBOARD_API_URL}/environment/", requestData))['env']['uuid']
                    }

                    // Get available nodes in our environment
                    nodes = common.parseJSON(http.sendHttpGetRequest("${DASHBOARD_API_URL}/environment/${cloudId}/nodes/"))
                    for (node in nodes) {
                        if (node['name'] == minion) {
                            nodeId = node['id']
                            break
                    ...
                    if (! nodeId ) {
                        // Create node
                        requestData['nodes'] = [minion]
                        nodeId = common.parseJSON(http.sendHttpPutRequest("${DASHBOARD_API_URL}/environment/${cloudId}/nodes/", requestData))['uuid']
                    }
            ...
            for (benchmark in benchmarksArray) {
                ...
                if (UPLOAD_TO_DASHBOARD.toBoolean()) {
                    ...
                    def reportId = common.parseJSON(http.sendHttpPostRequest("${DASHBOARD_API_URL}/reports/openscap/", requestData))['report']['uuid']

                    // Upload results
                    requestData['results'] = salt.GetFileContent(pepperEnv, minion, "${resultsDir}/results.json")
                    requestData['node_name'] = minion
                    http.sendHttpPutRequest("${DASHBOARD_API_URL}/reports/openscap/${reportId}/", requestData)
                }
            }
        }
    }
